...
## [2.3.0] - 2022-04-28
### Improvements
  * Added an `assignedTo` field to each Card
  * Added dropdown list `assignedToList` to permit changes to `assignedTo`. `assignedToList` is dictionary.
  * dropdown triggers call to `update({assignedTo:value})`
